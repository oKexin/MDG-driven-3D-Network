...
if __name__ == '__main__':
    ...
    generator.eval()
    generator.load_state_dict(torch.load('Result/proposed_{}x_Y.pth'.format(scale_factor), map_location=device))
    transform = transforms.Compose([
        transforms.ToTensor(),
    ])
    lr_frames = []
    num = 1
    lr_path = 'testdata_Y/dataset{}x/data{}'.format(scale_factor, num)
    hr_path = 'testdata_Y/HR/{}_warped.png'.format(num)
    for t in range(5):
    ...
    plt.axis('off')
    plt.savefig('Result/Yao/HR.png', dpi=300, bbox_inches='tight', pad_inches=0)
    plt.figure()
    plt.imshow(result, cmap='afmhot')
    plt.axis('off')
    plt.savefig('Result/Yao/{}x/propose.png'.format(scale_factor), dpi=300, bbox_inches='tight', pad_inches=0)
    plt.show()
    #Count Index
    hr_frame = transforms.ToTensor()(hr_frame).unsqueeze(0).to(device)
    lpips_loss_fn = lpips.LPIPS(net='vgg', verbose=False).to(device)
    lpips_value = lpips_loss_fn(hr_pred, hr_frame).item()
    psnr_count = PSNRCalculator().to(device)
    ssim_count = StructuralSimilarityIndexMeasure(data_range=1.0, reduction=None).to(device)
    psnr = psnr_count(hr_pred, hr_frame).item()
    ssim = ssim_count(hr_pred, hr_frame).item()
    print(f"SSIM: {ssim:.4f}, PSNR: {psnr:.2f}, LPIPS: {lpips_value:.4f}")

    image_uint8 = (result * 255).astype(np.uint8)
    save_path = 'Result/Yao/{}x/raw_propose.png'.format(scale_factor)
